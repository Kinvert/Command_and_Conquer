...
static unsigned int timeGetTime(void)
{
    return (unsigned int)(time(NULL) * 1000u);
}
...
extern "C" __declspec(dllexport) bool __cdecl CNC_TD_Micro_Configure(const char* ruleset,
                                                                      unsigned int seed);
extern "C" __declspec(dllexport) bool __cdecl CNC_TD_Micro_Configure_Difficulty(
    const char* ruleset,
    unsigned int seed,
...
extern "C" __declspec(dllexport) bool __cdecl CNC_TD_Micro_Configure(const char* ruleset, unsigned int seed)
{
    return TDMicro.Configure_Oracle(ruleset, seed);
}
...
extern "C" __declspec(dllexport) bool __cdecl CNC_TD_Micro_Configure_Difficulty(
    ...
    int opponent_difficulty)
{
    return TDMicro.Configure_Oracle_Difficulty(ruleset, seed, opponent_difficulty);
}
                                                                       ...
                                                                       int max_players)
{
    ...
    if (TDMicro.Enabled()) {
        if (scenario_index != 1 || num_players != 2 || player_list == NULL || player_list[0].IsAI
            || !player_list[1].IsAI || player_list[0].House != HOUSE_GOOD || player_list[1].House != HOUSE_GOOD
            || (game_options.MPlayerUnitCount != 0
                && game_options.MPlayerUnitCount != TD_MICRO_STARTING_FORCE_UNIT_COUNT)) {
            return false;
        }
        game_options.MPlayerBases = 1;
        game_options.MPlayerCredits = TD_MICRO_INITIAL_CREDITS;
        game_options.MPlayerTiberium = 0;
        game_options.MPlayerGoodies = 0;
        game_options.MPlayerSolo = 1;
        game_options.IsMCVDeploy = true;
        game_options.SpawnVisceroids = false;
        game_options.EnableSuperweapons = false;
        game_options.CaptureTheFlag = false;
        game_options.DestroyStructures = true;
        Scen.Difficulty = DIFF_NORMAL;
        Scen.CDifficulty = static_cast<DiffType>(TDMicro.Opponent_Handicap());
    }
    ...
    const int simulation_player_count = TDMicro.Enabled() ? 1 : num_players;
    MPlayerCount = simulation_player_count;           // # of human players in this game
    ...
    for (int i = 0; i < simulation_player_count; i++) {
...
void GlyphX_Assign_Houses(void)
{
    ...
    */
    ...
    for (i = 0; i < MPlayerCount; i++) {
        j = TDMicro.Enabled() ? 1 - i : Random_Pick(0, MPlayerMax - 1);
        ...
        */
        ...
        if (TDMicro.Enabled()) {
            housep->Assign_Handicap(
                housep->IsHuman ? DIFF_NORMAL : static_cast<DiffType>(TDMicro.Opponent_Handicap()));
        }
    ...
    if (TDMicro.Enabled() && MPlayerCount == 1 && MPlayerGhosts == 1) {
        const int house_slot = 0;
        house = static_cast<HousesType>(house_slot + static_cast<int>(HOUSE_MULTI1));
        housep = HouseClass::As_Pointer(house);
        if (housep != NULL) {
            house_used[house_slot] = true;
            color_used[REMAP_LTBLUE] = true;
            memset(housep->Name, 0, MPLAYER_NAME_MAX);
            strncpy(housep->Name, "OriginalAI", MPLAYER_NAME_MAX - 1);
            housep->IsHuman = false;
            housep->Init_Data(REMAP_LTBLUE, HOUSE_GOOD, MPlayerCredits);
            housep->Assign_Handicap(static_cast<DiffType>(TDMicro.Opponent_Handicap()));
            housep->StartLocationOverride = -1;
            MPlayerHouses[1] = house;
        }
    }
                                                                           ...
                                                                           const char* override_map_name)
{
    ...
    // Hack a fix for scenario 21 since the same mission number is used in Covert Ops and N64
    Scen.Scenario = (scenario_index == 81) ? 21 : scenario_index;
    BuildLevel = TDMicro.Enabled() ? 7 : build_level;
    ...
    if (stricmp(game_type, "GAME_NORMAL") == 0) {
        ...
        if (stricmp(game_type, "GAME_GLYPHX_MULTIPLAYER") == 0) {
            GameToPlay = TDMicro.Enabled() ? GAME_SKIRMISH : GAME_GLYPHX_MULTIPLAYER;
            ScenPlayer = SCEN_PLAYER_MPLAYER;
        } else {
            return false;
        }
    ...
    */
    ...
    Seed = TDMicro.Has_Oracle_Seed() ? TDMicro.Oracle_Seed() : timeGetTime();
    if (TDMicro.Enabled()) Init_Random();

    if (!Start_Scenario(Scen.ScenarioName)) {
        return (false);
    }
    if (TDMicro.Enabled()) Scen.RandomNumber.Seed = TDMicro.Reset_RNG_State();
...
extern "C" __declspec(dllexport) bool __cdecl CNC_Advance_Instance(uint64 player_id)
{
    ...
    */
    ...
    static bool FirstUpdate = !TDMicro.Enabled() && GameToPlay != GAME_GLYPHX_MULTIPLAYER;
    ;
    if (!FirstUpdate) {
        HouseClass* old_player_ptr = PlayerPtr;
    ...
    */
    if (TDMicro.Enabled() || GameToPlay == GAME_NORMAL) {
        Queue_AI();
    } else {
        if (GameToPlay == GAME_GLYPHX_MULTIPLAYER) {
    ...
    */
    if (GameToPlay != GAME_NORMAL && MPlayerGhosts) {
        if (TDMicro.Enabled()) {
            if (Random_Pick(0, 10000) == 1) Computer_Message();
        } else if (IRandom(0, 10000) == 1) {
