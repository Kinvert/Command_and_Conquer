...
int Start_TDMicro_Policy_Skirmish()
{
    /* The simulator/oracle contract is multiplayer scenario 1. Picking vector element zero can
       select scenario 0 when that file is installed, yielding a different map with no Tiberium at
       the 344 compact-observation cells generated from scenario 1. */
    bool scenario_one_available = false;
    for (int index = 0; index < MPlayerFilenum.Count(); ++index) {
        if (MPlayerFilenum[index] == 1) {
            scenario_one_available = true;
            break;
        }
    }
    if (!scenario_one_available) {
        std::fprintf(stderr, "TD Micro: multiplayer scenario 1 is unavailable\n");
        return false;
    }

    TDMicro_Policy_Reset();
    std::strcpy(MPlayerName, "TDMicro");
    MPlayerHouse = HOUSE_GOOD;
    MPlayerPrefColor = REMAP_GOLD;
    MPlayerColorIdx = REMAP_GOLD;
    BuildLevel = 7;
    /* TD Micro's curriculum rolls a constrained 2,300-credit start for 35% of episodes, which is
       what forces an economy game rather than a rush. The bridge has no curriculum clock, so the
       starting purse is set at launch via TD_MICRO_STARTING_CREDITS. Unset keeps the stock 10,000
       so existing runs are unaffected; "constrained" is accepted as an alias for 2,300. */
    MPlayerCredits = TD_MICRO_INITIAL_CREDITS;
    {
        const char* credits_value = std::getenv("TD_MICRO_STARTING_CREDITS");
        if (credits_value != NULL && credits_value[0] != '\0') {
            if (std::strcmp(credits_value, "constrained") == 0) {
                MPlayerCredits = TD_MICRO_STARTING_CREDITS_CONSTRAINED;
            } else {
                char* end = NULL;
                const long parsed = std::strtol(credits_value, &end, 10);
                if (end != credits_value && *end == '\0' && parsed > 0 && parsed <= TD_MICRO_INITIAL_CREDITS) {
                    MPlayerCredits = static_cast<int>(parsed);
                } else {
                    std::fprintf(stderr,
                                 "TD Micro: ignoring invalid TD_MICRO_STARTING_CREDITS=%s\n",
                                 credits_value);
                }
            }
        }
    ...
    MPlayerBases = 1;
    /* This switch controls whether the scenario's initial Tiberium overlays are loaded at all.
       Growth/spread are separate flags below and remain disabled, matching the finite training
       field. Setting this to zero produced an empty map where no Refinery could ever earn income. */
    MPlayerTiberium = 1;
    MPlayerGoodies = 0;
    MPlayerGhosts = 1;
    MPlayerUnitCount = TDMicro.Starting_Unit_Count();
    Special.IsCaptureTheFlag = 0;
    Special.IsEarlyWin = 1;
    Special.IsTGrowth = 0;
    Special.IsTSpread = 0;
    ScenarioIdx = 0;
    Seed = TDMicro.Setup_Seed();

    MPlayerCount = 1;
    MPlayerLocalID = Build_MPlayerID(MPlayerColorIdx, MPlayerHouse);
    MPlayerID[0] = MPlayerLocalID;
    std::strcpy(MPlayerNames[0], MPlayerName);
    Scen.Scenario = 1;
    Scen.CDifficulty = static_cast<DiffType>(TDMicro.Opponent_Handicap());
    Scen.Difficulty = DIFF_NORMAL;

    static char const* const difficulty_names[] = {"Easy", "Normal", "Hard"};
    std::fprintf(stderr,
                 "TD Micro: auto-starting scenario=%d seed=%u spawn=%s GDI policy vs %s GDI AI\n",
                 Scen.Scenario,
                 Seed,
                 TDMicro.Spawn_Bucket() == TD_MICRO_SPAWN_CLOSE ? "close" : "medium",
                 difficulty_names[TDMicro.Opponent_Difficulty()]);
    return true;
}
...
{
    if (TDMicro.Auto_Start_Skirmish()) return Start_TDMicro_Policy_Skirmish();
    ...
    */
    ...
    if (rc) {
        ...
        .....................................................................*/
        Scen.Scenario = MPlayerFilenum[TDMicro.Enabled() ? 0 : ScenarioIdx];
