...
bool HouseClass::Can_Build(TechnoTypeClass const* type, HousesType house) const
{
    ...
    if (TDMicro.Enabled()) {
        ...
        case RTTI_BUILDINGTYPE:
            allowed = TDMicro.Allows_Production(
                TD_MICRO_CATEGORY_BUILDING, static_cast<BuildingTypeClass const*>(type)->Type, IsHuman);
            break;
        case RTTI_INFANTRYTYPE:
            allowed = TDMicro.Allows_Production(
                TD_MICRO_CATEGORY_INFANTRY, static_cast<InfantryTypeClass const*>(type)->Type, IsHuman);
            break;
        case RTTI_UNITTYPE:
            allowed = TDMicro.Allows_Production(
                TD_MICRO_CATEGORY_UNIT, static_cast<UnitTypeClass const*>(type)->Type, IsHuman);
            break;
        default:
            break;
        }
...
void HouseClass::AI(void)
{
    ...
    if (TDMicro.Enabled() && GameToPlay != GAME_NORMAL) {
        ...
        for (int player = 0; player < MPlayerCount; ++player) {
            if (Class->House == MPlayerHouses[player]) {
                is_participant = true;
                break;
            }
        }
        if (TDMicro.Is_Skirmish_Opponent_House(Class->House, GameToPlay, MPlayerGhosts)) {
            is_participant = true;
        }
        if (!is_participant) {
            return;
