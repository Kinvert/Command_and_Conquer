...
static int TDMicro_Opponent_Starting_Credits()
{
    const char* value = std::getenv("TD_MICRO_OPPONENT_STARTING_CREDITS");
    if (!TDMicro.Auto_Start_Skirmish() || value == NULL || value[0] == '\0') return MPlayerCredits;
    errno = 0;
    char* end = NULL;
    const long parsed = std::strtol(value, &end, 10);
    if (errno != 0 || end == value || *end != '\0' || parsed < 0 || parsed > MPlayerCredits) {
        std::fprintf(stderr,
                     "TD Micro: ignoring invalid TD_MICRO_OPPONENT_STARTING_CREDITS=%s\n",
                     value);
        return MPlayerCredits;
    }
    std::fprintf(stderr, "TD Micro: opponent starting credits=%ld\n", parsed);
...
static void Assign_Houses(void)
{
    ...
    */
    for (i = 0; i < MPlayerCount; i++) {
        const int house_slot = TDMicro.Auto_Start_Skirmish() ? 1 - i : i;
        /*
        **	If this house was already selected, decrement 'i' & keep looping.
        */
        if (house_used[house_slot]) {
            i--;
            continue;
        }
        ...
        */
        house = (HousesType)(house_slot + (int)HOUSE_MULTI1);
    ...
    */
    for (i = MPlayerCount; i < MPlayerCount + MPlayerGhosts; i++) {
        const int house_slot = TDMicro.Auto_Start_Skirmish() ? 1 - i : i;
        if (house_used[house_slot] == false) {
            ...
            */
            house = (HousesType)(house_slot + (int)HOUSE_MULTI1);
            pref_house = TDMicro.Enabled() ? HOUSE_GOOD : (HousesType)(Random_Pick(0, 1) + (int)HOUSE_GOOD);
            if (TDMicro.Auto_Start_Skirmish()) {
                color = REMAP_LTBLUE;
            } else {
                for (;;) {
            ...
            */
            ...
            housep->Init_Data(color, pref_house, TDMicro_Opponent_Starting_Credits());

            DiffType difficulty =
                TDMicro.Enabled() ? static_cast<DiffType>(TDMicro.Opponent_Handicap()) : Scen.CDifficulty;

            if (!TDMicro.Enabled() && Players.Count() > 1 && Rule.IsCompEasyBonus && difficulty > DIFF_EASY) {
...
static void Create_Units(void)
{
    ...
    ........................................................................*/
    tot_units = TDMicro.Enabled() ? 0 : (MPlayerUnitCount * 2) / 3;
    ...
    ........................................................................*/
    tot_infantry = TDMicro.Enabled() ? 0 : MPlayerUnitCount - tot_units;
    ...
    for (h = HOUSE_MULTI1; h < (HOUSE_MULTI1 + MPlayerMax); h++) {
        ...
        if (TDMicro.Enabled()) {
            const bool player = hptr->IsHuman;
            int x = 0;
            int y = 0;
            if (!TDMicro.Start_Coordinate(player, x, y)) return;
            centroid = XY_Cell(Map.MapCellX + x, Map.MapCellY + y);
        } else {
    ...
    if (TDMicro.Enabled() && MPlayerUnitCount != 0 && !TDMicro_Add_Starting_Force()) {
        std::fprintf(stderr,
                     "TD Micro: failed to create deterministic starting force for unit count %d\n",
                     MPlayerUnitCount);
    }
}
