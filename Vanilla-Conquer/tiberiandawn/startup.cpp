...
#else
    const char* argv0_override = getenv("VANILLA_CONQUER_ARGV0");
    if (argv0_override && argv0_override[0] != 0) {
        strncpy(path_to_exe, argv0_override, sizeof(path_to_exe) - 1);
        path_to_exe[sizeof(path_to_exe) - 1] = 0;
    } else {
        ssize_t readed = readlink("/proc/self/exe", &path_to_exe[0], sizeof(path_to_exe) - 1);
        if (readed < 0 || readed >= (ssize_t)(sizeof(path_to_exe) - 1)) {
            strcpy(path_to_exe, "TiberianDawn.so");
        } else {
            path_to_exe[readed] = 0;
        }
    }
#endif
