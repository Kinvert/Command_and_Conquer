...
def test_cudagraph_warmup_loss_accumulator_is_cleared():
    from pufferlib.pufferl import _resolve_backend

    args = _load_args()
    if ENV_NAME == "cnc_micro":
        args["env"]["action_scheme"] = 1
    ...
    for name, value in loss.items():
        assert math.isfinite(value), f"initial loss/{name} is not finite: {value}"
        assert abs(value) < 1e-7, (
            f"CUDA-graph warmup leaked into initial loss/{name}: {value}"
        )
...
def _run_and_recover(anneal):
    ...
    # Drop the first couple recorded epochs to preserve this test's historical
    # comparison window.
