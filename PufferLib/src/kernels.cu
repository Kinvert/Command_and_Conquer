...
__global__ void cast_scaled(precision_t* __restrict__ dst,
        const unsigned char* __restrict__ src, int n, float scale) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        dst[idx] = from_float((float)src[idx] * scale);
    }
}

inline void cast_scaled_dispatch(precision_t* dst, const unsigned char* src,
        int n, float scale, cudaStream_t stream) {
    cast_scaled<<<grid_size(n), BLOCK_SIZE, 0, stream>>>(dst, src, n, scale);
}
