...
#include "muon.cu"
#include "vecenv.h"
#include "cnc_micro_action_spec.h"
#include "cnc_micro_group_action_spec.h"

static void cnc_micro_decoder_init_weights(void* weights, ulong* seed, cudaStream_t stream) {
    decoder_init_weights(weights, seed, stream);
    DecoderWeights* decoder = (DecoderWeights*)weights;
    assert(decoder->output_dim + 1 == cnc_micro_action::kDecoderOutputSize);
    const size_t row_count = cnc_micro_action::kZeroInitLogitEnd
        - cnc_micro_action::kZeroInitLogitBegin;
    cudaMemsetAsync(
        decoder->weight.data
            + cnc_micro_action::kZeroInitLogitBegin * decoder->hidden_dim,
        0,
        row_count * decoder->hidden_dim * sizeof(precision_t),
        stream);
}
...
struct RolloutBuf {
    ...
    PrecisionTensor component_logprobs; // Optional (horizon, agents, num_atns)
...
void register_rollout_buffers(RolloutBuf& bufs, Allocator* alloc, int T, int B, int input_size,
        int num_atns, int mask_size, int component_logprob_count) {
    bufs = (RolloutBuf){
    ...
    if (component_logprob_count > 0) {
        bufs.component_logprobs = {.shape = {T, B, component_logprob_count}};
        alloc_register(alloc, &bufs.component_logprobs);
    }
...
struct TrainGraph {
    ...
    PrecisionTensor mb_component_logprobs; // Optional (B, T, num_atns)
...
void register_train_buffers(TrainGraph& bufs, Allocator* alloc, int B, int T, int input_size,
    ...
    if (component_logprob_count > 0) {
        bufs.mb_component_logprobs = {.shape = {B, T, component_logprob_count}};
        alloc_register(alloc, &bufs.mb_component_logprobs);
    }
...
struct PPOGraphArgs {
    ...
    const precision_t* old_component_logprobs;
...
typedef struct {
    ...
    int num_threads;
    int seed;
    std::string training_state_fingerprint;
} HypersT;
...
typedef struct {
    ...
    int action_abi;  // Explicit CNC Micro policy ABI; zero for other environments.
...
struct CncMicroLogitReader {
    const precision_t* logits;
    int logits_base;

    __device__ __forceinline__ float operator()(int offset) const {
        return finite_or_clamp(
            safe_logit(logits, logits_base, offset, 0),
            -1.0e6f, 1.0e6f);
    }
};

__device__ __forceinline__ float cnc_micro_logit(
        const precision_t* logits, int logits_base,
        cnc_micro_action::LogitSpec spec, int token) {
    const CncMicroLogitReader read = {logits, logits_base};
    return finite_or_clamp(
        cnc_micro_action::score_logit(spec, token, read),
        -1.0e6f, 1.0e6f);
}

__device__ __forceinline__ bool cnc_micro_mask_allowed(
        const precision_t* mask, int mask_base, int bit_index) {
    if (mask == nullptr) return true;
    const int packed = static_cast<int>(to_float(mask[mask_base + bit_index / 8]));
    return cnc_micro_action::packed_mask_bit(packed, bit_index);
}

__device__ __forceinline__ int cnc_micro_sample_head(
        const precision_t* logits, int logits_base,
        cnc_micro_action::LogitSpec spec, int size,
        const precision_t* mask, int mask_base, int mask_offset,
        curandStatePhilox4_32_10_t* rng, float* logprob) {
    float max_logit = -INFINITY;
    float sum_exp = 0.0f;
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) continue;
        const float value = cnc_micro_logit(logits, logits_base, spec, token);
        if (value > max_logit) {
            sum_exp *= __expf(max_logit - value);
            max_logit = value;
        }
        sum_exp += __expf(value - max_logit);
    }
    const float logsumexp = max_logit + __logf(sum_exp);
    ...
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) continue;
        const float value = cnc_micro_logit(logits, logits_base, spec, token);
        cumulative += __expf(value - logsumexp);
    ...
    if (selected < 0) {
        ...
        if (mask != nullptr) {
            for (int token = size - 1; token >= 0; --token) {
                if (cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) {
                    selected = token;
                    break;
                }
            }
    ...
    *logprob = cnc_micro_logit(logits, logits_base, spec, selected) - logsumexp;
    return selected;
}

__global__ void sample_cnc_micro_logits(
        PrecisionTensor dec_out,
        precision_t* __restrict__ actions,
        precision_t* __restrict__ logprobs,
    ...
    const int batch = dec_out.shape[0];
    if (index >= batch) return;

    curandStatePhilox4_32_10_t rng = rng_states[index];
    const int logits_stride = dec_out.shape[1];
    const int logits_base = index * logits_stride;
    const int mask_base = action_mask != nullptr ? index * mask_stride : 0;
    float total_logprob = 0.0f;
    float head_logprob = 0.0f;

    int selected[4];
    selected[0] = cnc_micro_sample_head(
        dec_out.data, logits_base, command_logit_spec(), kCommandCount,
        action_mask, mask_base, kCommandMask, &rng, &head_logprob);
    total_logprob += head_logprob;

    for (int stage = 0; stage < 3; ++stage) {
        const int mask_offset = argument_mask_offset(
            selected[0], stage,
            stage >= 1 ? selected[1] : kPad,
            stage >= 2 ? selected[2] : kPad);
        const LogitSpec spec = argument_logit_spec(
            selected[0], stage,
            stage >= 1 ? selected[1] : kPad,
            stage >= 2 ? selected[2] : kPad);
        selected[stage + 1] = cnc_micro_sample_head(
            dec_out.data, logits_base, spec, kTokenCount,
            action_mask, mask_base, mask_offset, &rng, &head_logprob);
        total_logprob += head_logprob;
    }

    for (int head = 0; head < kActionHeadCount; ++head) {
        actions[index * kActionHeadCount + head] = from_float(selected[head]);
    }
    logprobs[index] = from_float(total_logprob);
    value_out[index] = dec_out.data[logits_base + kLogitCount];
    rng_states[index] = rng;
}

__device__ __forceinline__ float cnc_micro_group_logit(
        const precision_t* logits, int logits_base, int offset, int token) {
    return finite_or_clamp(
        safe_logit(logits, logits_base, offset + token, 0),
        -1.0e6f, 1.0e6f);
}

__device__ __forceinline__ bool cnc_micro_group_mask_allowed(
        const precision_t* mask, int mask_base, int offset, int token) {
    return mask == nullptr || to_float(mask[mask_base + offset + token]) != 0.0f;
}

__device__ __forceinline__ int cnc_micro_group_sample_head(
        const precision_t* logits, int logits_base, int logit_offset, int size,
        const precision_t* mask, int mask_base, int mask_offset,
        curandStatePhilox4_32_10_t* rng, float* logprob) {
    float max_logit = -INFINITY;
    float sum_exp = 0.0f;
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_group_mask_allowed(mask, mask_base, mask_offset, token)) continue;
        const float value = cnc_micro_group_logit(logits, logits_base, logit_offset, token);
        if (value > max_logit) {
            sum_exp *= __expf(max_logit - value);
            max_logit = value;
        }
    ...
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_group_mask_allowed(mask, mask_base, mask_offset, token)) continue;
        cumulative += __expf(
            cnc_micro_group_logit(logits, logits_base, logit_offset, token) - logsumexp);
        if (uniform < cumulative) {
            selected = token;
            break;
        }
    ...
    if (selected < 0) {
        for (int token = size - 1; token >= 0; --token) {
            if (cnc_micro_group_mask_allowed(mask, mask_base, mask_offset, token)) {
                selected = token;
                break;
            }
        }
    }
    *logprob = cnc_micro_group_logit(
        logits, logits_base, logit_offset, selected) - logsumexp;
    return selected;
}

__global__ void sample_cnc_micro_group_logits(
        PrecisionTensor dec_out,
        precision_t* __restrict__ actions,
        precision_t* __restrict__ logprobs,
        precision_t* __restrict__ component_logprobs,
        precision_t* __restrict__ value_out,
        curandStatePhilox4_32_10_t* __restrict__ rng_states,
        const precision_t* __restrict__ action_mask,
        int mask_stride) {
    ...
    if (index >= dec_out.shape[0]) return;

    curandStatePhilox4_32_10_t rng = rng_states[index];
    const int logits_base = index * dec_out.shape[1];
    const int mask_base = action_mask != nullptr ? index * mask_stride : 0;
    const int action_base = index * kActionHeadCount;
    float total_logprob = 0.0f;
    float head_logprob = 0.0f;

    const int command = cnc_micro_group_sample_head(
        dec_out.data, logits_base, kCommandOffset, kCommandCount,
        action_mask, mask_base, kCommandOffset, &rng, &head_logprob);
    actions[action_base + kCommand] = from_float(command);
    component_logprobs[action_base + kCommand] = from_float(head_logprob);
    total_logprob += head_logprob;

    if (command == kAttack) {
        for (int head = kActor; head < kTargetSlot; ++head) {
            actions[action_base + head] = from_float(forced_attack_value(head));
            component_logprobs[action_base + head] = from_float(0.0f);
        }
        int selected_count = 0;
        for (int slot = 0; slot < kSelectorCount; ++slot) {
            const int head = kBaseHeadCount + slot;
            const int offset = head_offset(head);
            if (!selector_contributes(
                    command,
                    cnc_micro_group_mask_allowed(action_mask, mask_base, offset, 1))) {
                actions[action_base + head] = from_float(0);
                component_logprobs[action_base + head] = from_float(0.0f);
                continue;
            }
            const int selected = cnc_micro_group_sample_head(
                dec_out.data, logits_base, offset, kSelectorValueCount,
                action_mask, mask_base, offset, &rng, &head_logprob);
            actions[action_base + head] = from_float(selected);
            component_logprobs[action_base + head] = from_float(head_logprob);
            total_logprob += head_logprob;
            selected_count += selected;
        }
        if (selected_count == 0) {
            actions[action_base + kTargetSlot] = from_float(0);
            component_logprobs[action_base + kTargetSlot] = from_float(0.0f);
        } else {
            const int target = cnc_micro_group_sample_head(
                dec_out.data, logits_base, kTargetSlotOffset, kTargetSlotCount,
                action_mask, mask_base, kAttackTargetMask, &rng, &head_logprob);
            actions[action_base + kTargetSlot] = from_float(target);
            component_logprobs[action_base + kTargetSlot] = from_float(head_logprob);
            total_logprob += head_logprob;
        }
    } else {
        for (int head = kActor; head < kBaseHeadCount; ++head) {
            const int selected = cnc_micro_group_sample_head(
                dec_out.data, logits_base, head_offset(head), head_size(head),
                action_mask, mask_base, head_offset(head), &rng, &head_logprob);
            actions[action_base + head] = from_float(selected);
            component_logprobs[action_base + head] = from_float(head_logprob);
            total_logprob += head_logprob;
        }
        for (int slot = 0; slot < kSelectorCount; ++slot) {
            actions[action_base + kBaseHeadCount + slot] = from_float(0);
            component_logprobs[action_base + kBaseHeadCount + slot] = from_float(0.0f);
        }
    }
...
extern "C" void net_callback_wrapper(void* ctx, int buf, int t) {
...
#ifdef OBS_SCALE
    cast_scaled_dispatch(obs_dst.data, obs_env.data + (long)start*obs_env.shape[1],
        n, OBS_SCALE, stream);
#else
    ...
    for (int b = 0; b < num_banks; b++) {
        ...
        PrecisionTensor component_lp_b = {};
        if (rollouts.component_logprobs.data != nullptr) {
            component_lp_b = puf_slice(
                rollouts.component_logprobs, t, sub_start, bank_size);
        }
        ...
        if (pufferl->action_abi == 13) {
            sample_cnc_micro_logits<<<grid_size(bank_size), BLOCK_SIZE, 0, stream>>>(
                dec_puf, act_b.data, lp_b.data, val_b.data,
                pufferl->rng_states[buf] + bank_off,
                mask_b.data, mask_stride_b);
        } else if (pufferl->action_abi == 14) {
            sample_cnc_micro_group_logits<<<grid_size(bank_size), BLOCK_SIZE, 0, stream>>>(
                dec_puf, act_b.data, lp_b.data, component_lp_b.data, val_b.data,
                pufferl->rng_states[buf] + bank_off,
                mask_b.data, mask_stride_b);
        } else {
...
__device__ __forceinline__ void cnc_micro_head_stats(
        const precision_t* logits, int logits_base,
        cnc_micro_action::LogitSpec spec, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float* logsumexp_out, float* entropy_out, float* logprob_out) {
    ...
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) continue;
        const float value = cnc_micro_logit(logits, logits_base, spec, token);
        if (token == selected) selected_logit = value;
        if (value > maximum) {
            sum *= __expf(maximum - value);
            maximum = value;
        }
        sum += __expf(value - maximum);
    }
    const float logsumexp = maximum + __logf(sum);
    float entropy = 0.0f;
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) continue;
        const float value = cnc_micro_logit(logits, logits_base, spec, token);
        const float logprob = value - logsumexp;
        const float probability = __expf(logprob);
        entropy -= probability * logprob;
    }
    *logsumexp_out = logsumexp;
    *entropy_out = entropy;
    *logprob_out = selected_logit - logsumexp;
}

__device__ __forceinline__ void cnc_micro_head_gradient(
        float* grad_logits, int grad_base,
        const precision_t* logits, int logits_base,
        cnc_micro_action::LogitSpec spec, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float logsumexp, float entropy, float d_new_logprob, float d_entropy) {
    for (int token = 0; token < size; ++token) {
        if (!cnc_micro_mask_allowed(mask, mask_base, mask_offset + token)) continue;
        const float value = cnc_micro_logit(logits, logits_base, spec, token);
        const float logprob = value - logsumexp;
        const float probability = __expf(logprob);
        const float gradient = cnc_micro_action::categorical_logit_gradient(
            token, selected, probability, logprob,
            entropy, d_new_logprob, d_entropy);
        for (int component = 0; component < spec.additive.count; ++component) {
            grad_logits[grad_base + spec.additive.base[component] + token] += gradient;
        }
        if (spec.interaction.enabled() && token < cnc_micro_action::kEntitySlotCount) {
            const CncMicroLogitReader read = {logits, logits_base};
            const float interaction_gradient_scale =
                cnc_micro_action::actor_target_gradient_scale(
                    spec.interaction, token, read);
            const int query = spec.interaction.query_base
                + spec.interaction.actor * cnc_micro_action::kActorTargetRank;
            const int key = spec.interaction.key_base
                + token * cnc_micro_action::kActorTargetRank;
            for (int rank = 0; rank < cnc_micro_action::kActorTargetRank; ++rank) {
                grad_logits[grad_base + query + rank] += gradient *
                    cnc_micro_action::actor_query_gradient_factor(
                        spec.interaction, token, rank,
                        interaction_gradient_scale, read);
                grad_logits[grad_base + key + rank] += gradient *
                    cnc_micro_action::target_key_gradient_factor(
...
__global__ void cnc_micro_ppo_loss_compute(
        float* __restrict__ ppo_partials,
        PPOKernelArgs a, PPOGraphArgs g) {
    using namespace cnc_micro_action;
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int thread = threadIdx.x;
    const int total = a.N * a.T_seq;
    const float inverse_total = 1.0f / float(total);

    __shared__ float block_losses[LOSS_N][PPO_THREADS];
    for (int loss = 0; loss < LOSS_N; ++loss) block_losses[loss][thread] = 0.0f;
    if (index >= total) goto reduce;

    {
        const int n = index / a.T_seq;
        const int t = index % a.T_seq;
        const int nt = n * a.T_seq + t;
        const int logits_base = n * a.logits_stride_n + t * a.logits_stride_t;
        const int value_index = n * a.values_stride_n + t * a.values_stride_t;
        const int grad_base = nt * kLogitCount;
        const int mask_base = a.action_mask != nullptr
            ? n * a.mask_stride_n + t * a.mask_stride_t : 0;

        int selected[kActionHeadCount];
        for (int head = 0; head < kActionHeadCount; ++head) {
            selected[head] = static_cast<int>(g.actions[nt * kActionHeadCount + head]);
        }

        LogitSpec specs[kActionHeadCount];
        int mask_offsets[kActionHeadCount];
        int sizes[kActionHeadCount] = {kCommandCount, kTokenCount, kTokenCount, kTokenCount};
        specs[0] = command_logit_spec();
        mask_offsets[0] = kCommandMask;
        for (int stage = 0; stage < 3; ++stage) {
            specs[stage + 1] = argument_logit_spec(
                selected[0], stage,
                stage >= 1 ? selected[1] : kPad,
                stage >= 2 ? selected[2] : kPad);
            mask_offsets[stage + 1] = argument_mask_offset(
                selected[0], stage,
                stage >= 1 ? selected[1] : kPad,
                stage >= 2 ? selected[2] : kPad);
        }

        float head_logsumexp[kActionHeadCount];
        float head_entropy[kActionHeadCount];
        float total_logprob = 0.0f;
        float total_entropy = 0.0f;
        for (int head = 0; head < kActionHeadCount; ++head) {
            float logprob;
            cnc_micro_head_stats(
                a.logits, logits_base, specs[head], sizes[head], selected[head],
                a.action_mask, mask_base, mask_offsets[head],
                &head_logsumexp[head], &head_entropy[head], &logprob);
            total_logprob += logprob;
            total_entropy += head_entropy[head];
        }

        const float old_logprob = to_float(g.old_logprobs[nt]);
        const float advantage = to_float(g.advantages[nt]);
        const float priority = to_float(g.prio[n]);
        const float old_value = to_float(g.values[nt]);
        const float return_value = to_float(g.returns[nt]);
        const float predicted_value = to_float(a.values_pred[value_index]);
        g.out_newvalue[nt] = from_float(predicted_value);

        const float advantage_std = sqrtf(float(a.adv_var[0]));
        const float normalized_advantage =
            (advantage - float(a.adv_mean[0])) / (advantage_std + 1.0e-8f);
        const float entropy_coefficient = *a.ent_coef;
        const float d_entropy = inverse_total * (-entropy_coefficient);

        const float value_error = predicted_value - old_value;
        const float clipped_value = old_value +
            fmaxf(-a.vf_clip_coef, fminf(a.vf_clip_coef, value_error));
        const float value_loss_unclipped =
            (predicted_value - return_value) * (predicted_value - return_value);
        const float value_loss_clipped =
            (clipped_value - return_value) * (clipped_value - return_value);
        const float value_loss = 0.5f * fmaxf(value_loss_unclipped, value_loss_clipped);
        float d_predicted_value = 0.0f;
        if (value_loss_clipped > value_loss_unclipped) {
            if (value_error >= -a.vf_clip_coef && value_error <= a.vf_clip_coef) {
                d_predicted_value = clipped_value - return_value;
            }
        } else {
            d_predicted_value = predicted_value - return_value;
        }
        a.grad_values_pred[nt] = inverse_total * a.vf_coef * d_predicted_value;

        const float logratio = total_logprob - old_logprob;
        const float ratio = __expf(logratio);
        g.out_ratio[nt] = from_float(ratio);
        const float clipped_ratio =
            fmaxf(1.0f - a.clip_coef, fminf(1.0f + a.clip_coef, ratio));
        const float weighted_advantage = -priority * normalized_advantage;
        const float policy_loss_unclipped = weighted_advantage * ratio;
        const float policy_loss_clipped = weighted_advantage * clipped_ratio;
        const float policy_loss = fmaxf(policy_loss_unclipped, policy_loss_clipped);
        float d_ratio = weighted_advantage * inverse_total;
        if (policy_loss_clipped > policy_loss_unclipped &&
                (ratio <= 1.0f - a.clip_coef || ratio >= 1.0f + a.clip_coef)) {
            d_ratio = 0.0f;
        }
        const float d_new_logprob = d_ratio * ratio;

        for (int head = 0; head < kActionHeadCount; ++head) {
            cnc_micro_head_gradient(
                a.grad_logits, grad_base,
                a.logits, logits_base, specs[head], sizes[head], selected[head],
                a.action_mask, mask_base, mask_offsets[head],
                head_logsumexp[head], head_entropy[head], d_new_logprob, d_entropy);
        }

        const float total_loss =
            (policy_loss + a.vf_coef * value_loss - entropy_coefficient * total_entropy) *
            inverse_total;
        block_losses[LOSS_PG][thread] = policy_loss * inverse_total;
        block_losses[LOSS_VF][thread] = value_loss * inverse_total;
        block_losses[LOSS_ENT][thread] = total_entropy * inverse_total;
        block_losses[LOSS_TOTAL][thread] = total_loss;
        block_losses[LOSS_OLD_APPROX_KL][thread] = (-logratio) * inverse_total;
        block_losses[LOSS_APPROX_KL][thread] = ((ratio - 1.0f) - logratio) * inverse_total;
        block_losses[LOSS_CLIPFRAC][thread] =
            (fabsf(ratio - 1.0f) > a.clip_coef ? 1.0f : 0.0f) * inverse_total;
    }
    ...
    for (int stride = PPO_THREADS / 2; stride > 0; stride >>= 1) {
        if (thread < stride) {
            for (int loss = 0; loss < LOSS_N; ++loss) {
                block_losses[loss][thread] += block_losses[loss][thread + stride];
            }
        }
        __syncthreads();
    }
    if (thread == 0) {
        const int base = blockIdx.x * (LOSS_N + 1);
        ppo_partials[base] = block_losses[LOSS_TOTAL][0];
        for (int loss = 0; loss < LOSS_N; ++loss) {
            ppo_partials[base + 1 + loss] = block_losses[loss][0];
        }
    }
}
...
void ppo_loss_fwd_bwd(
    ...
    PPOGraphArgs graph_args = {
        ...
        .old_component_logprobs = nullptr,
...
void cnc_micro_ppo_loss_fwd_bwd(
        PrecisionTensor& dec_out,
        TrainGraph& graph,
        FloatTensor& losses_acc,
        float clip_coef, float vf_clip_coef, float vf_coef, const float* ent_coef,
        PPOBuffersPuf& bufs,
        cudaStream_t stream) {
    using namespace cnc_micro_action;
    const int N = dec_out.shape[0];
    const int T = dec_out.shape[1];
    const int fused_columns = dec_out.shape[2];
    const int total = N * T;
    assert(fused_columns == kDecoderOutputSize);

    ppo_var_mean<<<1, 256, 0, stream>>>(
        graph.mb_advantages.data,
        bufs.adv_scratch.data,
        bufs.adv_scratch.data + 1,
        numel(graph.mb_advantages.shape));

    const int ppo_grid = (total + PPO_THREADS - 1) / PPO_THREADS;
    static float* partials = nullptr;
    static int partial_capacity = 0;
    const int partial_count = ppo_grid * (LOSS_N + 1);
    if (partials == nullptr || partial_count > partial_capacity) {
        if (partials != nullptr) cudaFree(partials);
        partial_capacity = partial_count;
        cudaMalloc(&partials, (size_t)partial_capacity * sizeof(float));
    }
    ...
    cudaMemsetAsync(
        ...
        0,
        (size_t)numel(bufs.grad_logits.shape) * sizeof(float),
        stream);
    ...
    const bool has_mask = graph.mb_action_mask.data != nullptr;
    const int mask_size = has_mask ? graph.mb_action_mask.shape[2] : 0;
    PPOKernelArgs args = {
        .grad_logits = bufs.grad_logits.data,
        .grad_logstd = nullptr,
        .grad_values_pred = bufs.grad_values.data,
        .logits = dec_out.data,
        .logstd = nullptr,
        .values_pred = dec_out.data + kLogitCount,
        .adv_mean = bufs.adv_scratch.data + 1,
        .adv_var = bufs.adv_scratch.data,
        .act_sizes = nullptr,
        .action_mask = has_mask ? graph.mb_action_mask.data : nullptr,
        .mask_stride_n = has_mask ? T * mask_size : 0,
        .mask_stride_t = has_mask ? mask_size : 0,
        .num_atns = kActionHeadCount,
        .clip_coef = clip_coef,
        ...
        .A_total = kLogitCount,
        .N = N,
        .logits_stride_n = T * fused_columns,
        .logits_stride_t = fused_columns,
        .logits_stride_a = 1,
        .values_stride_n = T * fused_columns,
        .values_stride_t = fused_columns,
        .is_continuous = false,
    };

    cnc_micro_ppo_loss_compute<<<ppo_grid, PPO_THREADS, 0, stream>>>(
        partials, args, graph_args);
    ppo_loss_reduce<<<1, LOSS_N + 1, 0, stream>>>(
        bufs.loss_output.data, losses_acc.data, partials, ppo_grid);
}

__device__ __forceinline__ void cnc_micro_group_head_stats(
        const precision_t* logits, int logits_base, int logit_offset, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float* logsumexp_out, float* entropy_out, float* logprob_out) {
    float maximum = -INFINITY;
    float sum = 0.0f;
...
__device__ __forceinline__ void cnc_micro_group_head_gradient(
        float* grad_logits, int grad_base,
        const precision_t* logits, int logits_base, int logit_offset, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float logsumexp, float entropy, float d_new_logprob, float d_entropy) {
    for (int token = 0; token < size; ++token) {
        ...
        const float logprob =
            cnc_micro_group_logit(logits, logits_base, logit_offset, token) - logsumexp;
        const float probability = __expf(logprob);
        grad_logits[grad_base + logit_offset + token] =
...
struct CncMicroGroupPpoAccumulator {
    float new_logprob;
    float old_logprob;
    float entropy;
    float policy_loss;
...
__device__ __forceinline__ void cnc_micro_group_accumulate_ppo_head(
        CncMicroGroupPpoAccumulator* accumulator,
        const precision_t* logits, int logits_base, int logit_offset, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float old_logprob, float weighted_advantage, float clip_coefficient) {
    float logsumexp;
    float entropy;
    float new_logprob;
    cnc_micro_group_head_stats(
        logits, logits_base, logit_offset, size, selected,
        mask, mask_base, mask_offset,
        &logsumexp, &entropy, &new_logprob);
    const cnc_micro_group_action::PpoHeadTerms terms =
        cnc_micro_group_action::ppo_head_terms(
        new_logprob, old_logprob, weighted_advantage, clip_coefficient);
    accumulator->new_logprob += new_logprob;
    accumulator->old_logprob += old_logprob;
    accumulator->entropy += entropy;
    accumulator->policy_loss += terms.policy_loss;
    accumulator->old_approx_kl += terms.old_approx_kl;
    accumulator->approx_kl += terms.approx_kl;
    accumulator->clip_fraction += terms.clip_fraction;
    ++accumulator->active_heads;
}

__device__ __forceinline__ void cnc_micro_group_ppo_head_gradient(
        float* grad_logits, int grad_base,
        const precision_t* logits, int logits_base, int logit_offset, int size, int selected,
        const precision_t* mask, int mask_base, int mask_offset,
        float old_logprob, float weighted_advantage, float clip_coefficient,
        float inverse_total, float d_entropy) {
    float logsumexp;
    float entropy;
    float new_logprob;
    cnc_micro_group_head_stats(
    ...
    const cnc_micro_group_action::PpoHeadTerms terms =
        cnc_micro_group_action::ppo_head_terms(
        new_logprob, old_logprob, weighted_advantage, clip_coefficient);
    cnc_micro_group_head_gradient(
        grad_logits, grad_base,
        logits, logits_base, logit_offset, size, selected,
        mask, mask_base, mask_offset,
        logsumexp, entropy, inverse_total * terms.d_new_logprob, d_entropy);
}

__global__ void cnc_micro_group_ppo_loss_compute(
        float* __restrict__ ppo_partials,
        PPOKernelArgs a, PPOGraphArgs g) {
    using namespace cnc_micro_group_action;
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
...

    {
        ...
        const int mask_base = a.action_mask != nullptr
            ? n * a.mask_stride_n + t * a.mask_stride_t : 0;
        const int action_base = nt * kActionHeadCount;
        const int command = static_cast<int>(to_float(g.actions[action_base + kCommand]));
        ...
        const float weighted_advantage = -priority * normalized_advantage;
        CncMicroGroupPpoAccumulator accumulator = {};
        cnc_micro_group_accumulate_ppo_head(
            &accumulator,
            a.logits, logits_base, kCommandOffset, kCommandCount, command,
            a.action_mask, mask_base, kCommandOffset,
            to_float(g.old_component_logprobs[action_base + kCommand]),
            weighted_advantage, a.clip_coef);
        if (command == kAttack) {
            int selected_count = 0;
            for (int slot = 0; slot < kSelectorCount; ++slot) {
                const int head = kBaseHeadCount + slot;
                const int selected =
                    static_cast<int>(to_float(g.actions[action_base + head]));
                const int offset = head_offset(head);
                if (!selector_contributes(
                        command,
                        cnc_micro_group_mask_allowed(
                            a.action_mask, mask_base, offset, 1))) continue;
                cnc_micro_group_accumulate_ppo_head(
                    &accumulator,
                    a.logits, logits_base, offset, kSelectorValueCount, selected,
                    a.action_mask, mask_base, offset,
                    to_float(g.old_component_logprobs[action_base + head]),
                    weighted_advantage, a.clip_coef);
                selected_count += selected;
            }
            if (selected_count != 0) {
                const int target =
                    static_cast<int>(to_float(g.actions[action_base + kTargetSlot]));
                cnc_micro_group_accumulate_ppo_head(
                    &accumulator,
                    a.logits, logits_base, kTargetSlotOffset, kTargetSlotCount, target,
                    a.action_mask, mask_base, kAttackTargetMask,
                    to_float(g.old_component_logprobs[action_base + kTargetSlot]),
                    weighted_advantage, a.clip_coef);
            }
        } else {
            for (int head = kActor; head < kBaseHeadCount; ++head) {
                ...
                cnc_micro_group_accumulate_ppo_head(
                    &accumulator,
                    a.logits, logits_base, head_offset(head), head_size(head), selected,
                    a.action_mask, mask_base, head_offset(head),
                    to_float(g.old_component_logprobs[action_base + head]),
                    weighted_advantage, a.clip_coef);
            }
        }
        ...
        const float joint_logratio = bounded_ppo_logratio(
            accumulator.new_logprob - accumulator.old_logprob);
        const float ratio = __expf(joint_logratio);
        g.out_ratio[nt] = from_float(ratio);
        const float policy_loss = centered_subaction_policy_loss(
            accumulator.policy_loss, accumulator.active_heads, weighted_advantage);

        cnc_micro_group_ppo_head_gradient(
            a.grad_logits, grad_base,
            a.logits, logits_base, kCommandOffset, kCommandCount, command,
            a.action_mask, mask_base, kCommandOffset,
            to_float(g.old_component_logprobs[action_base + kCommand]),
            weighted_advantage, a.clip_coef, inverse_total, d_entropy);
        if (command == kAttack) {
            int selected_count = 0;
            for (int slot = 0; slot < kSelectorCount; ++slot) {
                const int head = kBaseHeadCount + slot;
            ...
            for (int head = kActor; head < kBaseHeadCount; ++head) {
                const int selected =
                    static_cast<int>(to_float(g.actions[action_base + head]));
                const int offset = head_offset(head);
                const int size = head_size(head);
                cnc_micro_group_ppo_head_gradient(
                    a.grad_logits, grad_base,
                    a.logits, logits_base, offset, size, selected,
                    a.action_mask, mask_base, offset,
                    to_float(g.old_component_logprobs[action_base + head]),
                    weighted_advantage, a.clip_coef, inverse_total, d_entropy);
            }
        ...
        const float total_loss =
            (policy_loss + a.vf_coef * value_loss -
             entropy_coefficient * accumulator.entropy) *
            inverse_total;
        block_losses[LOSS_PG][thread] = policy_loss * inverse_total;
        block_losses[LOSS_VF][thread] = value_loss * inverse_total;
        block_losses[LOSS_ENT][thread] = accumulator.entropy * inverse_total;
        block_losses[LOSS_TOTAL][thread] = total_loss;
        block_losses[LOSS_OLD_APPROX_KL][thread] =
            accumulator.old_approx_kl * inverse_total;
        block_losses[LOSS_APPROX_KL][thread] =
            accumulator.approx_kl * inverse_total;
        block_losses[LOSS_CLIPFRAC][thread] =
            (accumulator.clip_fraction / float(accumulator.active_heads)) * inverse_total;
    }
...
void cnc_micro_group_ppo_loss_fwd_bwd(
        PrecisionTensor& dec_out,
        TrainGraph& graph,
        FloatTensor& losses_acc,
        float clip_coef, float vf_clip_coef, float vf_coef, const float* ent_coef,
    ...
    const int total = N * T;
    assert(fused_columns == kDecoderOutputSize);
    assert(graph.mb_component_logprobs.data != nullptr);
    assert(graph.mb_component_logprobs.shape[2] == kActionHeadCount);
    ...
    PPOGraphArgs graph_args = {
        ...
        .actions = graph.mb_actions.data,
        .old_logprobs = graph.mb_logprobs.data,
        .old_component_logprobs = graph.mb_component_logprobs.data,
        .advantages = graph.mb_advantages.data,
        .prio = graph.mb_prio.data,
        .values = graph.mb_values.data,
        .returns = graph.mb_returns.data,
    ...
    cnc_micro_group_ppo_loss_compute<<<ppo_grid, PPO_THREADS, 0, stream>>>(
        partials, args, graph_args);
    ppo_loss_reduce<<<1, LOSS_N + 1, 0, stream>>>(
        bufs.loss_output.data, losses_acc.data, partials, ppo_grid);
}
...
__global__ void select_copy(RolloutBuf rollouts, TrainGraph graph,
    ...
    int component_lp_row_bytes = rollouts.component_logprobs.data != nullptr
        ? (numel(rollouts.component_logprobs.shape)
            / rollouts.component_logprobs.shape[0]) * sizeof(precision_t)
        : 0;
    ...
    case 5:
        ...
        } else if (graph.mb_component_logprobs.data != nullptr) {
            copy_bytes((const char*)rollouts.component_logprobs.data,
                       (char*)graph.mb_component_logprobs.data,
                       src_row, mb, component_lp_row_bytes);
        }
        break;
    case 6:
        if (graph.mb_action_mask.data != nullptr &&
                graph.mb_component_logprobs.data != nullptr) {
            copy_bytes((const char*)rollouts.component_logprobs.data,
                       (char*)graph.mb_component_logprobs.data,
                       src_row, mb, component_lp_row_bytes);
        }
...
void train_impl(PuffeRL& pufferl) {
    ...
    if (src.component_logprobs.data != nullptr) {
        int component_count = src.component_logprobs.shape[2];
        transpose_102<<<grid_size(T*B*component_count), BLOCK_SIZE, 0, train_stream>>>(
            rollouts.component_logprobs.data, src.component_logprobs.data,
            T, B, component_count);
    }
    transpose_102<<<grid_size(T*B), BLOCK_SIZE, 0, train_stream>>>(
    ...
    int total_epochs = hypers.schedule_timesteps / batch_size;
    if (anneal_lr) {
    ...
    for (int mb = 0; mb < total_minibatches; ++mb) {
        ...
        if (hypers.reset_state) puf_zero(&graph.mb_state, train_stream);
        {
            ...
            int channels = 5
                + (graph.mb_action_mask.data != nullptr ? 1 : 0)
                + (graph.mb_component_logprobs.data != nullptr ? 1 : 0);
        ...
        if (pufferl.train_captured) {
            ...
            if (pufferl.action_abi == 13) {
                cnc_micro_ppo_loss_fwd_bwd(
                    dec_puf, graph, pufferl.losses_puf,
                    hypers.clip_coef, hypers.vf_clip_coef, hypers.vf_coef,
                    pufferl.ppo_bufs_puf.ent_coef.data,
                    pufferl.ppo_bufs_puf, stream);
            } else if (pufferl.action_abi == 14) {
                cnc_micro_group_ppo_loss_fwd_bwd(
                    dec_puf, graph, pufferl.losses_puf,
                    hypers.clip_coef, hypers.vf_clip_coef, hypers.vf_coef,
                    pufferl.ppo_bufs_puf.ent_coef.data,
                    pufferl.ppo_bufs_puf, stream);
...
static Policy build_policy(const char* env_name, int input_size, int hidden_size,
                           int num_layers, int network_type,
                           int decoder_output_size, int act_n,
                           bool is_continuous, int horizon, int action_abi) {
    Encoder encoder = {
    ...
    if (action_abi == 13) {
        decoder.init_weights = cnc_micro_decoder_init_weights;
    }
    Network network = {
    ...
    if (network_type == NATIVE_NETWORK_MLP) {
        network = Network{
            .forward = mlp_forward,
            .forward_train = mlp_forward_train,
            .backward = mlp_backward,
            .init_weights = mlp_init_weights,
            .reg_params = mlp_reg_params,
            .reg_train = mlp_reg_train,
            .reg_rollout = mlp_reg_rollout,
            .create_weights = mlp_create_weights,
            .free_weights = mlp_free_weights,
            .free_activations = mlp_free_activations,
            .hidden = hidden_size, .num_layers = num_layers, .horizon = horizon,
            .activation_size = sizeof(MLPActivations),
        };
    } else if (network_type != NATIVE_NETWORK_MINGRU) {
        throw std::runtime_error("unsupported native network type");
    }
    return Policy{
...
static void weight_bank_create_for_pufferl(WeightBank* bank, PuffeRL* pufferl,
    ...
    if (!pufferl->is_continuous && pufferl->action_abi == 13) {
        decoder_output_size = cnc_micro_action::kLogitCount;
    }
    bank->policy = build_policy(pufferl->env_name.c_str(), input_size, hidden_size,
        num_layers, pufferl->hypers.network_type, decoder_output_size, act_n,
        pufferl->is_continuous, pufferl->hypers.horizon, pufferl->action_abi);
...
static bool cnc_micro_action_schema_matches(
        int abi, int head_count, const int* sizes, int mask_size) {
    // Mirrors policy_abi9.action_head_sizes. The product head is CNC26's nine-wide version; taking
    // it from the group-action spec keeps this from drifting when the head changes again.
    static const int abi9_sizes[] = {12, 65, cnc_micro_group_action::kProductCount, 4, 64, 64, 64};
    static const int abi9_mask_size =
        12 + 65 + cnc_micro_group_action::kProductCount + 4 + 64 + 64 + 64;
    static const int abi13_sizes[] = {12, 65, 65, 65};
    if (abi == 9) {
        if (head_count != 7 || mask_size != abi9_mask_size) return false;
        for (int head = 0; head < head_count; ++head) {
            if (sizes[head] != abi9_sizes[head]) return false;
        }
        return true;
    }
    if (abi == 13) {
        if (head_count != 4 || mask_size != cnc_micro_action::kMaskSize) return false;
        for (int head = 0; head < head_count; ++head) {
            if (sizes[head] != abi13_sizes[head]) return false;
        }
        return true;
    }
    if (abi == 14) {
        if (head_count != cnc_micro_group_action::kActionHeadCount ||
                mask_size != cnc_micro_group_action::kMaskSize) return false;
        for (int head = 0; head < 7; ++head) {
            if (sizes[head] != abi9_sizes[head]) return false;
        }
        for (int head = 7; head < head_count; ++head) {
            if (sizes[head] != cnc_micro_group_action::kSelectorValueCount) return false;
        }
        return true;
    }
    return false;
...
std::unique_ptr<PuffeRL> create_pufferl_impl(HypersT& hypers,
    ...
    pufferl->vec = vec;
    pufferl->action_abi = env_name == "cnc_micro" ? get_action_abi() : 0;
    if (env_name == "cnc_micro" &&
            pufferl->action_abi != 9 &&
            pufferl->action_abi != 13 &&
            pufferl->action_abi != 14) {
        throw std::runtime_error("cnc_micro returned an unsupported action ABI");
    }
    ...
    if (env_name == "cnc_micro" &&
            !cnc_micro_action_schema_matches(
                pufferl->action_abi,
                num_action_heads,
                raw_act_sizes,
                pufferl->vec->action_mask_size)) {
        throw std::runtime_error("cnc_micro action ABI does not match its head/mask schema");
    }
    int act_n = 0;
    ...
    if (!is_continuous && pufferl->action_abi == 13) {
        decoder_output_size = cnc_micro_action::kLogitCount;
    }
    ...
    pufferl->policy = build_policy(pufferl->env_name.c_str(), input_size, hidden_size,
        num_layers, hypers.network_type, decoder_output_size, act_n,
        is_continuous, hypers.horizon, pufferl->action_abi);
    ...
    if (hypers.cudagraphs >= 0) {
        ...
        // Capture warmup is not part of training telemetry.
        cudaMemset(pufferl->losses_puf.data, 0, NUM_LOSSES * sizeof(float));
        cudaDeviceSynchronize();
