...
enum NativeNetworkType {
    NATIVE_NETWORK_MINGRU = 0,
    NATIVE_NETWORK_MLP = 1,
};
...
// Feedforward control for architecture ablations. This mirrors the Python MLP
// block shape (Linear + GELU) while staying on the native CUDA training path.
struct MLPWeights {
    int hidden, num_layers, horizon;
    PrecisionTensor* weights;
    PrecisionTensor* biases;
};

struct MLPActivations {
    int num_layers;
    PrecisionTensor* saved_inputs;
    PrecisionTensor* pre_activations;
    PrecisionTensor* outputs;
    PrecisionTensor* grad_pre;
    PrecisionTensor* grad_inputs;
    PrecisionTensor* wgrad_scratch;
    PrecisionTensor* bgrad_scratch;
};

__global__ void mlp_bias_gelu_kernel(
        precision_t* __restrict__ output,
        precision_t* __restrict__ pre_activation,
        const precision_t* __restrict__ bias,
        int total, int hidden) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= total) return;
    float x = to_float(pre_activation[idx]) + to_float(bias[idx % hidden]);
    // Backward needs the exact biased argument passed to GELU.
    pre_activation[idx] = from_float(x);
    float cdf = 0.5f * (1.0f + erff(x * 0.7071067811865475f));
    output[idx] = from_float(x * cdf);
}

__global__ void mlp_gelu_backward_kernel(
        precision_t* __restrict__ grad_input,
        const precision_t* __restrict__ grad_output,
        const precision_t* __restrict__ pre_activation,
        int total) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= total) return;
    float x = to_float(pre_activation[idx]);
    float cdf = 0.5f * (1.0f + erff(x * 0.7071067811865475f));
    float pdf_term = x * expf(-0.5f * x * x) * 0.3989422804014327f;
    grad_input[idx] = from_float(to_float(grad_output[idx]) * (cdf + pdf_term));
}

__global__ void mlp_bias_grad_kernel(
        precision_t* __restrict__ bias_grad,
    ...
    for (int row = 0; row < rows; ++row) {
        sum += to_float(grad[row * hidden + column]);
    }
    bias_grad[column] = from_float(sum);
}

static void mlp_init_weights(void* weights, ulong* seed, cudaStream_t stream) {
    MLPWeights* m = (MLPWeights*)weights;
    for (int i = 0; i < m->num_layers; ++i) {
        puf_kaiming_init(&m->weights[i], std::sqrt(2.0f), (*seed)++, stream);
        puf_zero(&m->biases[i], stream);
    }
}

static void mlp_reg_params(void* weights, Allocator* alloc) {
    MLPWeights* m = (MLPWeights*)weights;
    for (int i = 0; i < m->num_layers; ++i) {
        m->weights[i] = {.shape = {m->hidden, m->hidden}};
        m->biases[i] = {.shape = {m->hidden}};
        alloc_register(alloc, &m->weights[i]);
        alloc_register(alloc, &m->biases[i]);
    }
}

static void mlp_allocate_arrays(MLPActivations* a, int num_layers) {
    a->num_layers = num_layers;
    a->saved_inputs = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->pre_activations = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->outputs = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->grad_pre = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->grad_inputs = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->wgrad_scratch = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
    a->bgrad_scratch = (PrecisionTensor*)calloc(num_layers, sizeof(PrecisionTensor));
}

static void mlp_reg_train(
        void* weights, void* activations, Allocator* acts, Allocator* grads, int B_TT) {
    MLPWeights* m = (MLPWeights*)weights;
    MLPActivations* a = (MLPActivations*)activations;
    int B = B_TT / m->horizon;
    mlp_allocate_arrays(a, m->num_layers);
    for (int i = 0; i < m->num_layers; ++i) {
        a->saved_inputs[i] = {.shape = {B, m->horizon, m->hidden}};
        a->pre_activations[i] = {.shape = {B, m->horizon, m->hidden}};
        a->outputs[i] = {.shape = {B, m->horizon, m->hidden}};
        a->grad_pre[i] = {.shape = {B, m->horizon, m->hidden}};
        a->grad_inputs[i] = {.shape = {B, m->horizon, m->hidden}};
        a->wgrad_scratch[i] = {.shape = {m->hidden, m->hidden}};
        a->bgrad_scratch[i] = {.shape = {m->hidden}};
        alloc_register(acts, &a->saved_inputs[i]);
        alloc_register(acts, &a->pre_activations[i]);
        alloc_register(acts, &a->outputs[i]);
        alloc_register(acts, &a->grad_pre[i]);
        alloc_register(acts, &a->grad_inputs[i]);
        alloc_register(grads, &a->wgrad_scratch[i]);
        alloc_register(grads, &a->bgrad_scratch[i]);
    }
}

static void mlp_reg_rollout(void* weights, void* activations, Allocator* alloc, int B) {
    MLPWeights* m = (MLPWeights*)weights;
    MLPActivations* a = (MLPActivations*)activations;
    mlp_allocate_arrays(a, m->num_layers);
    for (int i = 0; i < m->num_layers; ++i) {
        a->pre_activations[i] = {.shape = {B, m->hidden}};
        a->outputs[i] = {.shape = {B, m->hidden}};
        alloc_register(alloc, &a->pre_activations[i]);
        alloc_register(alloc, &a->outputs[i]);
    }
}

static void* mlp_create_weights(void* self) {
    Network* network = (Network*)self;
    MLPWeights* weights = (MLPWeights*)calloc(1, sizeof(MLPWeights));
    weights->hidden = network->hidden;
    weights->num_layers = network->num_layers;
    weights->horizon = network->horizon;
    weights->weights = (PrecisionTensor*)calloc(network->num_layers, sizeof(PrecisionTensor));
    weights->biases = (PrecisionTensor*)calloc(network->num_layers, sizeof(PrecisionTensor));
    return weights;
}

static void mlp_free_weights(void* weights) {
    MLPWeights* m = (MLPWeights*)weights;
    free(m->weights);
    free(m->biases);
    free(m);
...
static void mlp_free_activations(void* activations) {
    MLPActivations* a = (MLPActivations*)activations;
    free(a->saved_inputs);
    free(a->pre_activations);
    free(a->outputs);
    free(a->grad_pre);
    free(a->grad_inputs);
    free(a->wgrad_scratch);
    free(a->bgrad_scratch);
    free(a);
}

static PrecisionTensor mlp_forward_impl(
        void* weights, PrecisionTensor x, void* activations,
        cudaStream_t stream, bool save_inputs) {
    MLPWeights* m = (MLPWeights*)weights;
    MLPActivations* a = (MLPActivations*)activations;
    for (int i = 0; i < m->num_layers; ++i) {
        if (save_inputs) puf_copy(&a->saved_inputs[i], &x, stream);
        puf_mm(&x, &m->weights[i], &a->pre_activations[i], stream);
        int total = numel(a->pre_activations[i].shape);
        mlp_bias_gelu_kernel<<<grid_size(total), BLOCK_SIZE, 0, stream>>>(
            a->outputs[i].data, a->pre_activations[i].data,
            m->biases[i].data, total, m->hidden);
        x = a->outputs[i];
    }
    return x;
}

static PrecisionTensor mlp_forward(
        void* weights, PrecisionTensor x, PrecisionTensor state,
        void* activations, cudaStream_t stream) {
    (void)state;
    return mlp_forward_impl(weights, x, activations, stream, false);
}

static PrecisionTensor mlp_forward_train(
        void* weights, PrecisionTensor x, PrecisionTensor state,
        void* activations, cudaStream_t stream) {
    (void)state;
    return mlp_forward_impl(weights, x, activations, stream, true);
}

static PrecisionTensor mlp_backward(
        void* weights, PrecisionTensor grad, void* activations, cudaStream_t stream) {
    MLPWeights* m = (MLPWeights*)weights;
    MLPActivations* a = (MLPActivations*)activations;
    for (int i = m->num_layers - 1; i >= 0; --i) {
        int total = numel(a->pre_activations[i].shape);
        mlp_gelu_backward_kernel<<<grid_size(total), BLOCK_SIZE, 0, stream>>>(
            a->grad_pre[i].data, grad.data, a->pre_activations[i].data, total);
        puf_mm_tn(
            &a->grad_pre[i], &a->saved_inputs[i], &a->wgrad_scratch[i], stream);
        mlp_bias_grad_kernel<<<grid_size(m->hidden), BLOCK_SIZE, 0, stream>>>(
            a->bgrad_scratch[i].data, a->grad_pre[i].data,
            total / m->hidden, m->hidden);
        puf_mm_nn(
            &a->grad_pre[i], &m->weights[i], &a->grad_inputs[i], stream);
        grad = a->grad_inputs[i];
    }
    return grad;
}
