...
elif [ "$ENV" = "cnc_micro" ]; then
    SRC_DIR="ocean/$ENV"
    TD_MICRO_DIR="${TD_MICRO_DIR:-$(cd ../td-micro && pwd)}"
    (cd "$TD_MICRO_DIR" && \
        ZIG_GLOBAL_CACHE_DIR="${ZIG_GLOBAL_CACHE_DIR:-/tmp/td-micro-zig-global-cache}" \
        zig build lib -Doptimize=ReleaseFast)
    INCLUDES+=(-I"$TD_MICRO_DIR/zig-out/include")
    EXTRA_LDFLAGS+=("$TD_MICRO_DIR/zig-out/lib/libtd_micro.a" -ldl -lpthread -lm)
...
OBS_SCALE=$(awk '/^#define OBS_SCALE/{print $3}' "$BINDING_SRC")
OBS_SCALE_DEFINE=()
if [ -n "$OBS_SCALE" ]; then
    OBS_SCALE_DEFINE=(-DOBS_SCALE="$OBS_SCALE")
fi
