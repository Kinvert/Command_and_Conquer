...
class Categorical(Space):
    def __init__(self, values, scale='auto'):
        if not isinstance(values, (list, tuple)) or len(values) < 2:
            raise ValueError('categorical values must contain at least two choices')
        self.values = tuple(values)
        self.min = self.values[0]
        self.max = self.values[-1]
        self.scale = 1.0 if scale == 'auto' else scale
        self.norm_min = -1.0
        self.norm_max = 1.0
        self.norm_mean = 0.0
    ...
    def normalize(self, value):
        try:
            ...
            raise ValueError(f'{value!r} is not in categorical values {self.values!r}') from exc
        return 2 * index / (len(self.values) - 1) - 1

    def unnormalize(self, value):
        zero_one = (value + 1) / 2
        index = round(zero_one * (len(self.values) - 1))
        return self.values[max(0, min(index, len(self.values) - 1))]

def _params_from_puffer_sweep(sweep_config, only_include=None, exclude=None, prefix=''):
    param_spaces = {}

    if 'sweep_only' in sweep_config:
        only_include = [p.strip() for p in sweep_config['sweep_only'].split(',')]
    if 'sweep_exclude' in sweep_config:
        exclude = [p.strip().replace('/', '.') for p in sweep_config['sweep_exclude'].split(',')]

    for name, param in sweep_config.items():
        if name in ('method', 'metric', 'metric_distribution', 'goal', 'downsample', 'use_gpu', 'prune_pareto',
                    'sweep_only', 'sweep_exclude', 'max_suggestion_cost', 'early_stop_quantile', 'gpus', 'max_runs',
                    'match_enemy_model_path', 'match_num_games', 'match_enemy_hidden_size',
                    'match_enemy_num_layers', 'workers_per_gpu'):
            continue

        assert isinstance(param, dict), f'Param {name} is not a dict'
        param_path = f'{prefix}.{name}' if prefix else name
        if any(isinstance(param[k], dict) for k in param):
            nested = _params_from_puffer_sweep(param, only_include, exclude, param_path)
            if nested:
                param_spaces[name] = nested
            continue
 
        if only_include and not any(
                k == name or k == param_path or k in name for k in only_include):
            continue
        if exclude and any(k == name or k == param_path for k in exclude):
            continue

        assert 'distribution' in param
        distribution = param['distribution']
        if distribution == 'categorical':
            space = Categorical(param['values'], param['scale'])
